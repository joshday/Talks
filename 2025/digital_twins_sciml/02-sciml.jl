...
# ╔═╡ c5697e8d-00b9-411a-8874-1d3085057ec7
begin 
	using PlutoUI, Plots, Cobweb
...
# ╔═╡ fb44c88e-a34b-47b1-9d61-0ba669625891
using DifferentialEquations

# ╔═╡ a76976b3-9183-4de3-9c4e-79d9f053a7de
using Symbolics

# ╔═╡ 51778f88-e405-48e5-9ace-e06cec81d249
using ModelingToolkit

# ╔═╡ 23ea723b-388c-418f-bbb6-cc99424cf0ec
using Surrogates

# ╔═╡ 45404d15-e7c0-4483-999b-f5f72740d03a
using Optimization

# ╔═╡ aa8b24f3-3a51-40fa-8588-c4e76aace97f
md"""
# SciML

#### What is it?

- A vast ecosystem for **Sci**entific Computing and **M**achine **L**earning. 
- Differentiable Modeling and Simulation Combined with Machine Learning.

> High performance is key. Any performance that is not at the top level is considered a bug and should be reported as such.
>
> - [SciML Docs](https://docs.sciml.ai/SciMLBase/stable/#Common-Interface-High-Level)
"""

# ╔═╡ 8ca30cfe-873a-4353-ab0b-230627b5dce8
md"""
!!! note "SciML Domains"
	- Linear Systems
	- Nonlinear Systems
...
# ╔═╡ 0f437d11-ace2-4589-b161-b17c462fbf93
md"""
## The Ecosystem

- 100+ Packages
- Some foundational, many are niche
"""

# ╔═╡ ecccd431-e1a0-4097-b2d9-3faa96dd0200
LocalResource("sciml.svg")

# ╔═╡ 219d5220-71ac-4436-b7a9-6d2c0e1aecd2
md"""
!!! note "SciML Core"
	#### Decoupled interfaces:
	
	| Concept                         | Interface / Type                                                                                  | Purpose                                                                      |
	| ------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
	| **Problem Definition**          | `AbstractSciMLProblem` (e.g., `ODEProblem`, `SDEProblem`, `DAEProblem`, `NonlinearProblem`, etc.) | Describes the mathematical model                                             |
	| **Solution**                    | `SciMLBase.AbstractSciMLSolution`                                                                 | Represents the computed solution (interpolable, time-indexed, etc.)          |
	| **Solvers**                     | `solve(prob, alg; kwargs...)`                                                                     | Dispatches to a chosen algorithm (explicit, implicit, adaptive, stiff, etc.) |
	| **Algorithms**                  | Subtypes of `SciMLBase.AbstractODEAlgorithm`, `AbstractDiffEqAlgorithm`, etc.                     | Pluggable solver methods (e.g., `Tsit5()`, `Rodas5()`)                       |
	| **Sensitivity Interface**       | `sensealg`, `adjoint_sensitivities`, etc.                                                         | For computing gradients of solutions with respect to parameters              |
	| **Callback System**             | `DiscreteCallback`, `ContinuousCallback`                                                          | Handle events, constraints, hybrid systems                                   |
	| **ModelingToolkit Integration** | Symbolic modeling via `ODESystem`, `DAESystem`, etc.                                              | Symbolic graph-based model composition, simplification, and code generation  |

"""

# ╔═╡ f18162fb-f50b-4e99-9d9c-b2fbb358e5fa
md"""
!!! note "Note on Multiple Dispatch"
	SciML relies heavily on multiple dispatch:
	- `solve(prob, alg; kw...)`, etc.
"""

# ╔═╡ d3be9e99-0ae1-4053-bf52-5c087180d8a6
md"""
## ODEs
"""

# ╔═╡ 730fdeed-0b1e-48d1-8ac5-e229e7ba1df0
let	
	f(u, p, t) = -p * u 	#  du/dt = f(u, p, t) 
	u0 = 1.0  				# initial condition of state variable `u`
	tspan = (0.0, 5.0) 		# time interval to solve over
	p = 0.5              	# decay rate parameter
	...
	# Plot
	plot(sol, ylim=(0,1), title="My First ODE: Exponential Decay")

	# Solutions are callable and use interpolation.
	scatter!(1:5, sol.(1:5))
end

# ╔═╡ 88089a25-c944-4b3f-97ef-70b59af632f3
md"""
## ODEs via Symbolics

### Symbolics.jl

- Powerful, modern CAS for Julia
- Primitives are written in Julia, so it's easy to extend.

"""

# ╔═╡ f954400b-f7f4-4941-a0a7-cfc34e83bbc1
let 
	@variables x y
	...
	@info b
	@info A'b
	@info Symbolics.derivative(h(x^2, 1), x)
end

# ╔═╡ 14ba7c44-e8b4-451f-b246-4d3a86866ccf
md"""
### ModelingToolkit

!!! note "Main Idea"
	Symbolics --> ODESystem, SDESystem, NonlinearSystem, ...

!!! success "Digital Twins"
	MTK is the core SciML package for developing digital twins.  Reusable components can be taken from the standard library or built from scratch to suit your needs.

	This notebook stops just shy of going into components, but if the other parts make sense to you, learning them is not a big lift.

---
- MTK automatically handles many computational efficiencies:
  - Generates fast functions.
  - Determines (and takes advantage of) Jacobian sparsity.
- Also lets you compose models through components ([link](https://docs.sciml.ai/ModelingToolkit/stable/tutorials/acausal_components/)).
  - See Also: [ModelingToolkitStandardLibrary](https://docs.sciml.ai/ModelingToolkitStandardLibrary/stable/).
"""

# ╔═╡ d5d9c306-8e3d-487c-a7c3-b721ccaf3aab
md"""
### Lotka-Volterra Predator-Prey Model

- States 
  - ``x`` is population density of prey.
  - ``y`` is population density of predator.
- Parameters:
  - ``\alpha`` is max prey per capita growth rate.
  - ``\beta`` is effect of predator presence on prey death rate.
  - ``\gamma`` is predator per capita death rate.
  - ``\delta`` is effect of prey presence on predator growth rate.
"""

# ╔═╡ 84352194-1a6b-44e6-9587-c0eb9bc244b2
md"""

| parameter | value |
|-----------|-------|
``\alpha`` | $(@bind α_input Slider(0:.1:5, default=1.5, show_value=true))
``\beta`` | $(@bind β_input Slider(0:.1:5, default=1.0, show_value=true))
``\gamma`` | $(@bind γ_input Slider(0:.1:5, default=3.0, show_value=true))
``\delta`` | $(@bind δ_input Slider(0:.1:5, default=1.0, show_value=true))
"""

# ╔═╡ 985bdc60-4bfa-43d2-bdda-3bff643bd00d
begin
	@independent_variables t
	
	@variables x(t)=1 y(t)=1 z(t)

	# input vars are Float64 from sliders
	@parameters α=α_input β=β_input γ=γ_input δ=δ_input
	...
	# generates function, computes Jacobians symbolically, etc.
	@mtkbuild sys = ODESystem(eqs, t)

	ode_prob = ODEProblem(sys, [], (0.0, 10.0))

	sol = solve(ode_prob)

	plot(
		plot(sol, title = "Rabbits vs Wolves"),
		plot(sol, idxs = z, title = "Total Animals"),
		layout=(2, 1)
	)
end

# ╔═╡ 87f9e9d2-4039-499c-bf96-1f09fa74e96f
sys

# ╔═╡ 6958092b-8876-49dd-8ced-52c563b1d177
md"""
## Calibration

- Estimating parameters to fit data.
- [Example in SciML Docs](https://docs.sciml.ai/Overview/stable/getting_started/fit_simulation/)

!!! note "Note the Interface"
	1. Create an `OptimizationProblem`.
	2. `solve` it.
"""

# ╔═╡ 7bdce102-6594-455d-818c-6855f695da3b
md"""
# Noteworthy Packages

- **DataDrivenDiffEq**: Finding systems of equations automatically from a dataset.
- **NeuralPDE**: Solving DEs with Physics-Informed Neural Networks (PINNs).
  - Solving PDESystems:
    - [Link](https://docs.sciml.ai/NeuralPDE/stable/tutorials/pdesystem/)
  - GPU Acceleration:
    - [Link](https://docs.sciml.ai/NeuralPDE/stable/tutorials/gpu/).
- **Surrogates**: Fast approximations that mimic expensive functions/simulations.
- **Catalyst**: Chemical reaction network and systems biology interface.
"""

# ╔═╡ 9ce797d2-fa9f-4e60-a4e6-8d310059a256
md"""
## NeuralPDE


- Solving DEs with Physics-Informed Neural Networks (PINNs):
  - [Link](https://docs.sciml.ai/NeuralPDE/stable/tutorials/pdesystem/)
- GPU Acceleration:
  - [Link](https://docs.sciml.ai/NeuralPDE/stable/tutorials/gpu/).
"""

# ╔═╡ f9680c01-86f7-41f9-b210-a1880a4a8b80
md"""
## Surrogates

- Approximations that mimic behavior of computationally expensive functions, e.g. simulations.
- Many ways to build surrogates:
  - radial basis functions, kriging, neural networks, ...
"""

# ╔═╡ ce0eb5d8-e203-4424-b939-160d8ee36bfe
md"""
#### 0) Pretend the [booth function](https://en.wikipedia.org/wiki/Test_functions_for_optimization) is computationally expensive.
"""

# ╔═╡ 2018f8d8-8edf-4e6f-978e-d7d2f689772b
function booth(x)
    x1 = x[1]
    x2 = x[2]
    term1 = (x1 + 2 * x2 - 7)^2
...
# ╔═╡ 5daba265-4def-462f-bff0-d899c93abc0c
md"""
#### 1) Sample points from expensive function
- Design of experiments plays a large role.
"""

# ╔═╡ 4bbe64ee-84bf-49a4-8173-c9eb05c60053
begin
	n_samples = 100
	lower_bound = [-5.0, 0.0]
...
# ╔═╡ 71d9f79d-70f8-47d3-a37c-6b8ab4bde2e8
let
	x, y = -5.0:10.0, 0.0:15.0
	p1 = surface(x, y, (x1, x2) -> booth((x1, x2)))
	xs = [xy[1] for xy in xys]
	ys = [xy[2] for xy in xys]
	sticks!(xs, ys, zs, marker=2, lab=nothing, title="True Function & Samples")
end

# ╔═╡ b7daccee-6973-41c8-a935-00940f74b853
md"""
#### 2) Build surrogate
"""

# ╔═╡ e131a9fe-bc1b-407e-8552-c4bb62666567
begin
	radial_basis = RadialBasis(xys, zs, lower_bound, upper_bound)
	@info "Check that it works:" booth(ones(2)) radial_basis(ones(2))
end

# ╔═╡ ab9782bd-38a2-446e-abe7-341df9c8b787
let
	x, y = -5.0:10.0, 0.0:15.0
	xs = [xy[1] for xy in xys]
	ys = [xy[2] for xy in xys]
	p1 = surface(x, y, (x, y) -> radial_basis([x y]), title="Surrogate")
	# scatter!(xs, ys, zs, marker=2, lab=nothing, title="Surrogate")
end

# ╔═╡ 93a82058-f2f5-490c-98ef-fb5036452ec1
md"""
#### 3) Optimization using the Surrogate

We can either:

1. Use the surrogate function `radial_basis` directly.
    - E.g. new samples are unavailable/too espensive to compute.
2. Use `surrogate_optimize`, which is allowed to add new samples.
"""

# ╔═╡ 5a07a9fd-801e-4dfb-a520-05261028008b
# Method 1: Use generic solver
let 
	f(u, p) = radial_basis(u)
	optf = OptimizationFunction(f, AutoForwardDiff())
	u0 = zeros(2)
	p = zeros(2)
	prob = OptimizationProblem(optf, u0, p)
	sol = solve(prob, Optimization.LBFGS())
	@info "Optimization.jl solution: $(sol.u)"
end

# ╔═╡ 53772e72-68ef-4a86-b714-c810b3e29c6d
# Method 2: Use surrogate_optimize
begin
	sur_opt = surrogate_optimize(booth, SRBF(), lower_bound, upper_bound, radial_basis, RandomSample(), maxiters=50)
	@info "surrogate_optimize solution: $(sur_opt[1])"
end

# ╔═╡ 901d7739-7639-441e-85ee-73020420aedc
sur_opt

# ╔═╡ 9e2d1f7f-6181-48ed-8428-f469f319cf72
let
	x, y = -5.0:10.0, 0.0:15.0
	xs = [xy[1] for xy in xys]
	ys = [xy[2] for xy in xys]
	p1 = surface(x, y, (x, y) -> radial_basis([x y]))
	scatter!(xs, ys, zs, marker=2, lab=nothing, title="Surrogate-Derived Optimum")
	scatter!([sur_opt[1][1]], [sur_opt[1][2]], [sur_opt[2]], label=nothing, m=7)
end

# ╔═╡ e218aa2d-0781-4ccd-af1e-45dba5deca65
md"""
# Thanks!
...
manifest_format = "2.0"
project_hash = "7b3f8d0e95e1ebe8fd414658895dfdca0f8efa7b"
